   ...
   for($i=0;$i<2;$i++)
   {
     ...
     $username = mysql_result($query_run,0, `us_un`);
      // $userid = mysql_result($query_run,0, 'us_pl');
       ...
       echo '<br>';
       ?>
<form action = 'messageindex.php' method = "GET">
    <input type = "submit" name= "message" value = "Send message">
    <input type = "hidden" name = "value_name" value = "<?php echo $username;?>">
</form>
             <?php
   }
}
...
{
    echo "You have no one to chat".'<br>';
    ?><a href="publicchat.php?username=<?php echo $us_un;?>">Enter into public chat</a><?php
}